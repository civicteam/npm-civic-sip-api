...
describe('Encryption and decryption', () => {
  ...
  it('should encrypt and decrypt a response using partner secret and AES', (done) => {
    const doneFn = done;
    const txt = '[{"label": "contact.personal.email","value": "test@tester.com","isValid": true,"isOwner": true}]';
  ...
  it('should create and verify JWT token with encrypted data and decrypt', (done) => {
    const doneFn = done;
  ...
  it('should try no more than four times to create the token', () => {
    const cipherText = basicCrypto.encrypt(userData, SECRET);
    const errorMessage = 'unknown ECDSA sig r length error';

    sinon.stub(rs.jws.JWS, 'sign').throws(errorMessage);
    ...
    try {
      ...
      expect(error.attempts).to.equal(4);
      expect(error.message).to.equal(errorMessage);
    }
  });
});

// describe('jsRsaSign JWTToken module', function test() {
//   this.timeout(10000);
...
//   it('should generate ES256 NIST keypair in PEM HEX and JWK formats', (done) => {
//     const doneFn = done;
...
//   it('should generate Civic ES256 NIST public key in PEM HEX and JWK formats', (done) => {
//     const doneFn = done;

//     const pubKey = new rs.KJUR.crypto.ECDSA({ curve });
...
//   it('should generate a long-lived token', (done) => {
//     const doneFn = done;

//     const prvKey = new rs.KJUR.crypto.ECDSA({ curve });
...
//   it('should compare javascript and php hmac sha256 results', (done) => {
//     const doneFn = done;
...
//     const hmacInBase64 = CryptoJS.enc.Base64.stringify(hmacBuffer);
...
//   it('should sign+verify token using JWK format', (done) => {
//     const doneFn = done;
...
//     // verify JWT
//     const isValid = rs.jws.JWS.verifyJWT(token, pubKey, { alg: [ALGO] });
...
//      */
//   it('should verify a JWT token using ES256 key in PEM format', (done) => {
//     const doneFn = done;
...
//     // verify JWT
//     const isValid = rs.jws.JWS.verifyJWT(authCode_ES256, pubPEM, { alg: [ALGO] });
//     console.log('Verified? : ', isValid);

//     doneFn();
//   });

//   it('should verify a JWT token using ES256 key in JWK format', (done) => {
...
//     const pubKeyFromHex = new rs.KJUR.crypto.ECDSA({ curve });
...
//   it('should verify a partner authorization header for auth code exchange api call.', (done) => {
...
//   it('should generate JWT token using ES256 Algorithm', (done) => {
//     const doneFn = done;
...
//     const decoded = rs.jws.JWS.parse(tokenES256);
